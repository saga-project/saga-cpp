...
#if !defined(SAGA_TEST_FILE_API_HPP)
#define SAGA_TEST_FILE_API_HPP
 ...
 namespace api
 {
    ...
    struct call_test_exception
    {
        ...
        int operator()(Tag)
        {
            ...
            equipment_.file = equipment_.utils->create_temp_file_for_exception(
                    (saga::error)E::value);
            equipment_.file.set_scheme(equipment_.scheme);
            equipment_.file.set_host(equipment_.host);
            SAGA_REQUIRE_THROW(TestFunctor::test(Tag(), E(), equipment_), err);
//             if(err == saga::NotImplemented) // NotImplemented
//             {
//               std::string message(TestFunctor::name);
...
//             else
            ...
            equipment_.utils->delete_temp_file(equipment_.file);
            return 0;
        }
