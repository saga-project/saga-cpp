...
static const char* rcsid = "@(#)$RCSfile: dh_sig.c,v $ $Revision: 1.1 $ (DOWNHILL) $Date: 2006/02/07 17:25:06 $";
#endif
		...
		else
		{
			 ...
			 SA_RESETHAND)
			{
				downhill_Signal_Info[signal_Number].
				 signal_Handler = (void (*)(int, ...)) SIG_DFL;
			}
#endif
...
static int downhill_Signal_Init(void)
{
	...
	if (downhill_Signal_Info == NULL)
	{
		...
		/* Get some memory */
		downhill_Signal_Info = (struct downhill_Signal_Struct *)
...
#undef signal
					downhill_Signal_Info[signal_Index].
					 signal_Handler = (void (*)(int, ...)) signal(signal_Index,
					 (void (*)(int)) downhill_Signal_Handle);
					break;
				default:
					downhill_Signal_Info[signal_Index].
					 signal_Handler = (void (*)(int, ...)) SIG_DFL;
     ...
     struct sigaction* sigaction_InfoOld)
{
	...
	switch (signal_Number)
	{
		DOWNHILL_SIGNAL_ALL
			...
			{
				sigaction_InfoOld->sa_handler = (void (*)(void))
				 downhill_Signal_Info[signal_Number].
				 signal_Handler;
			...
			{
				downhill_Signal_Info[signal_Number].
				 signal_Handler = (void (*)(int, ...)) sigaction_Info->sa_handler;
				downhill_Signal_Info[signal_Number].
				 signal_Count = 0;
				downhill_Signal_Info[signal_Number].
				 signal_Extra = 0;
