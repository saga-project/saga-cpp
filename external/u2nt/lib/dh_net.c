...
static const char* rcsid = "@(#)$RCSfile: dh_net.c,v $ $Revision: 1.1 $ (DOWNHILL) $Date: 2006/02/07 17:25:06 $";
#endif
...
retry:
	...
	else
	{
		...
		write(client_Stdout,temp_Port,(int)strlen(temp_Port)+1);
	...
	/* Send the command ------------------------------------------------ */
	send(client_Stdout,server_User,(int)strlen(server_User)+1,0);
	send(client_Stdout,server_Password,(int)strlen(server_Password)+1,0);
	send(client_Stdout,server_Command,(int)strlen(server_Command)+1,0);
     ...
     char* server_User,char* server_Command,int* client_Stderr)
{
	...
	*server_Host = server_Info->h_name;
	...
	else
	{
		...
		if (send(client_Stdout,sock_Port,(int)strlen(sock_Port)+1,0) !=
	...
	/* Send the command ------------------------------------------------ */
	send(client_Stdout,client_User,(int)strlen(client_User)+1,0);
	send(client_Stdout,server_User,(int)strlen(server_User)+1,0);
	send(client_Stdout,server_Command,(int)strlen(server_Command)+1,0);
