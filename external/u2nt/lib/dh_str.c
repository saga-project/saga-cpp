...
static const char* rcsid = "@(#)$RCSfile: dh_str.c,v $ $Revision: 1.1 $ (DOWNHILL) $Date: 2006/02/07 17:25:06 $";
#endif
...
#else
#defin strncasecmp(a, b, c) strnicmp(a, b, c)
#endif
...
int strcasecmp(char* string_One,char* string_Two)
{
	...
	return strncasecmp(string_One,string_Two,(int)strlen(string_One));
}
#endif
...
int ffs(int bit_Bitmap)
{
	...
	for (bit_Index = 0; bit_Index < sizeof(int)*8;bit_Index++)
	{
		...
		if (bit_Bitmap&(int)exp((double)bit_Index))
		{
			return bit_Index+1;
		}
	}
