...
static const char* rcsid = "@(#)$RCSfile: dh_env.c,v $ $Revision: 1.1 $ (DOWNHILL) $Date: 2006/02/07 17:25:06 $";
#endif
...
static int new_chunk()
{
  ...
  new_shadow_env = (shadow_env_t *)realloc(shadow_env,
...
static shadow_env_t* getshadow(const char* name)
{
  ...
  if ((mangled_name = (char *)malloc(shadow_length + 1)) == NULL) {
    return NULL;
  }
  ...
  if ((shadow_name = (char *)malloc(strlen(name)+1)) == NULL) {
    free(mangled_name);
    return NULL;
  }
  ...
  if ((mangled_value = getenv(mangled_name)) == NULL) {
    ...
    if ((unmangled_value = getenv(name)) == NULL) {
      ...
      real_assertion = (char *)realloc(mangled_name, shadow_length + 2);
       ...
       * version.
       */
      shadow_env[shadow_used].shadow_value = (char *)malloc(strlen(unmangled_value)+1);
    ...
    shadow_length += (int)strlen(mangled_value);
    real_assertion = (char *)realloc(mangled_name, shadow_length + 2);
    if (real_assertion == NULL) {
      free(shadow_name);
      free(mangled_name);
    ...
    shadow_env[shadow_used].shadow_value = (char *)malloc(strlen(demangle_value(mangled_value)) + 1);
    if (shadow_env[shadow_used].shadow_value == NULL) {
      free(shadow_name);
      free(mangled_name);
      return NULL;
...
setenv(const char *name, const char *value, int rewrite)
{
  ...
  if (rewrite || !shadow->shadow_value) {
    ...
    new_value = (char *)realloc(shadow->shadow_value, strlen(value) + 1);
    if (new_value == NULL) {
      return -1;
    }
    ...
    new_assertion = (char *)realloc(shadow->real_assertion,
      mangled_name_length
...
unsetenv(const char *name)
{
  ...
  if (i != shadow_used) {
    ...
    *++real_value = '\0';
    ...
    char* assertion = (char *)malloc(strlen(name)+2);
    if (assertion == NULL) {
      return;
    }
...
int Downhill_Putenv(char *option)
{
  int retval = -1;
  char *value = NULL;
  if (NULL == option)
