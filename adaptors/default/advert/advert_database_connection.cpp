...
namespace advert
{
    ...
    bool database_connection::verify_node_exists(std::string const& adname, int node_id)
    {
    // figure out, if the given advert entry is already given for this node
        int count = 0;
        eIndicator ind = eOK;
        sql_ << "select count(*) from nodes where node_id = :id",
            into(count, ind), use(node_id);
            
        if (ind != eOK) {
            SAGA_OSSTREAM strm;
            strm << "advert::advert_cpi_impl::verify_node_exists: "
    ...
    // retrieve node corresponding to the given adname
        ...
        if (0 == node_id || !verify_node_exists(adname, node_id)) {
            SAGA_OSSTREAM strm;
